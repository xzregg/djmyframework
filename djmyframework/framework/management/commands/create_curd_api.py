...
class ModelTemplateCreater(object):
    def __init__(self, app_name, model_name, model, params={}, options={}):
        ...
        self.params['app_verbose_name'] = apps.app_configs.get(self.app_name).verbose_name
    ...
    def create_model_v_file(self):
        template_file = 'controller.py.tpl'
        single_view_file = os.path.join(self.get_app_path(), 'controller.py')
        if os.path.isfile(single_view_file):
            target_file = single_view_file
        else:
            target_file = os.path.join(self.get_app_path(), 'controller', '%s.py' % self.model_lower_name)
        self._create_base_file(template_file, target_file)

    def create_model_services_file(self):
        template_file = 'services.py.tpl'
        single_service_file = os.path.join(self.get_app_path(), 'services.py')
        if os.path.isfile(single_service_file):
            target_file = single_service_file
        else:
            target_file = os.path.join(self.get_app_path(), 'services', '%s.py' % self.model_lower_name)
        self._create_base_file(template_file, target_file)

    def create_model_serializer_file(self):
        template_file = 'serializer.py.tpl'
        single_serializer_file = os.path.join(self.get_app_path(), 'serializer.py')
        if os.path.isfile(single_serializer_file):
            target_file = single_serializer_file
        else:
